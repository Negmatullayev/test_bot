...
async function generateCertificatePdf(certificateData) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc.image(qrBuffer, 137, height - 205, { width: 108, height: 108 });
      doc.fontSize(8)
         .fillColor('#526777')
         .text('QR ORQALI TEKSHIRISH', 125, height - 88, { width: 132, align: 'center', characterSpacing: 0.7 });
      doc.fontSize(8)
         .fillColor('#102a43')
         .text(certificateNumber, 125, height - 73, { width: 132, align: 'center' });

      // Original seal and signature area.
      const sealX = width - 208;
      const sealY = height - 164;
      ...
      doc.fontSize(8).fillColor('#526777').text(`Sana: ${issueDateText}`, width - 335, height - 214, { width: 190, align: 'right' });
      doc.moveTo(width - 338, height - 166).lineTo(width - 145, height - 166).lineWidth(1).stroke('#102a43');
      doc.fontSize(11).fillColor('#102a43').text('Bilim Sinovi jamoasi', width - 338, height - 155, { width: 193, align: 'right' });
      doc.fontSize(8).fillColor('#526777').text(`Telegram: @${botUsername}`, width - 338, height - 137, { width: 193, align: 'right' });
      doc.fontSize(8).fillColor('#1e7f91').text('Sertifikatni tekshirish', width - 338, height - 115, { width: 193, align: 'right', link: verifyUrl, underline: true });
      doc.fontSize(8).fillColor('#1e7f91').text('Botga o‘tish', width - 338, height - 100, { width: 193, align: 'right', link: botUrl, underline: true });
